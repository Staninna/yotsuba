...
- [ ] 100% e2e test coverage. 321 instrumented Compose tests in `app/src/androidTest/`, grouped by screen (`shell/`, `home/`, `boards/`, `catalog/`, `threads/`, `thread/`, `media/`, `settings/`, `vault/`, `stats/`), driving the real navigation graph against in-memory fakes for every repository, with a knob per failure and empty state. No network, no Room. CI runs them on an API 35 emulator on every push (`ci.yml`, job `instrumented`), about 18 minutes. The 2026-08-29 `NoSuchMethodError: runBlockingK` was not a stale artifact: the test APK took coroutines-test 1.11.0 from the catalog while the app resolved coroutines-core transitively to 1.10.2, and declaring `coroutines-android` in the app fixed it. First green run 2026-09-10 (`34507459366`, 321 tests, about 20 minutes of test time). It took eight CI passes to get there, 13 then 5 then 5 then 3 then 5 then 2 then 2 then 1 then 0 failures, and every fix but the orchestrator experiment stuck. It cannot be run on this laptop, because `/dev/kvm` belongs to whichever desktop session owns the seat, so accelerated runs die in ProbeKVM and `-accel off` takes about 5 minutes per test. A screen's composables can be driven on the JVM instead: Robolectric plus `compose-ui-test-junit4` on the unit-test classpath runs a real screen in seconds, which is how the swipe-undo data loss was finally caught (see `BookmarksListSwipeTest`). Instrumented line coverage read 29.2% before this suite landed (`./gradlew :app:createDebugAndroidTestCoverageReport`); re-measure after a full green run, and read it as UI-layer coverage only, because every repository is a fake so the data layer necessarily reads 0%
- [ ] What the e2e suite deliberately cannot reach, recorded by the agents who wrote it: anything that leaves the app (share chooser, browser, picture-in-picture, the system file picker, install-unknown-apps, all-files-access settings), which is asserted up to the tap; loading spinners and progress counters, because the fakes answer inside one frame; the navigation rail, which needs a non-compact window; the frame picker and the seek bar, which need a decodable video fixture; gallery hiding and the app lock, which need shared storage and a device credential; the loop handles on the seek bar and a board profile's typography, neither of which leaves a trace a test can read, and whether the link preview was fetched at all, which needs a `FakeLinkPreview` behind a small interface in main. Most of the rest is a fake knob away (a delay, a `Downloading` status) rather than a lost cause
- [ ] The failure family that cost five of the eight passes, quiet since pass 8 but not understood as fixed: "No compose hierarchies found in the app", "Detected multithreaded access to SnapshotStateObserver", "performMeasureAndLayout called during measure layout", a composition disposed mid-detach, and once a `ModalBottomSheet` built on `DefaultDispatcher-worker-6`. It is not one test leaking into the next: pass 4 gave every test its own process and cleared app data between them, and the family survived, at triple the run time. It is the harness, not the app. `compose-ui-test` 1.12.0 composes on an `UnconfinedTestDispatcher` (`AndroidComposeUiTestEnvironment.compositionCoroutineDispatcher`), and unconfined means a continuation resumes on whichever thread completed it, so `LaunchedEffect { value = withContext(Dispatchers.IO) { ... } }`, which the app does in `ImageTextSheet`, `FramePicker` and `MediaShare`, recomposes off the main thread under test and cannot under a user, whose recomposer is confined to the main thread for the life of the window. The environment takes a `useStandardTestDispatcherForComposition` flag that would fix it, but the constructor carrying it is `internal` and no public factory exposes it, so the lever is out of reach until androidx opens it up
...
- [ ] Re-measure unit coverage. The 22.8% line figure predates the test overhaul and the JVM suite is now roughly 600 tests; the instrumented half is recorded above
