...
- [ ] 100% e2e test coverage. 319 instrumented Compose tests in `app/src/androidTest/`, grouped by screen (`shell/`, `home/`, `boards/`, `catalog/`, `threads/`, `thread/`, `media/`, `settings/`, `vault/`, `stats/`), driving the real navigation graph against in-memory fakes for every repository, with a knob per failure and empty state. No network, no Room. CI runs them on an API 35 emulator on every push (`ci.yml`, job `instrumented`). The 2026-08-29 `NoSuchMethodError: runBlockingK` was not a stale artifact: the test APK took coroutines-test 1.11.0 from the catalog while the app resolved coroutines-core transitively to 1.10.2, and declaring `coroutines-android` in the app fixed it. Last whole-suite local run was 2026-09-10: 131 of the first 154 passed before the run died, all 23 failures were then fixed, and the re-run is unverified because `/dev/kvm` belongs to whichever desktop session owns the seat, so accelerated runs die in ProbeKVM. Adding the `stan` user to the `kvm` group and logging in again makes it permanent; `-accel off` boots but takes about 5 minutes per test, too slow for the suite. Instrumented line coverage read 29.2% before this suite landed (`./gradlew :app:createDebugAndroidTestCoverageReport`); re-measure after a full green run, and read it as UI-layer coverage only, because every repository is a fake so the data layer necessarily reads 0%
